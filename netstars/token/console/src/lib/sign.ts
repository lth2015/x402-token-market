 ...
 * sdk/src/netstars/transport.py `sign_request`. Mirrors haba's
 * src/lib/netstars/sign.ts (we keep two copies on purpose; see
 * docs/haba-technical-plan §5.1 on intentional code-duplication
 * between HABA site and Netstars Console).
 *
 * Server-only: uses node:crypto and reads HMAC secret from env.
 */
import "server-only";
...
export function signRequest(args: {
  method: string;
  /** Path only — must NOT include query string */
  path: string;
  body: string | Buffer;
  apiKeyId: string;
  apiKeySecret: string;
