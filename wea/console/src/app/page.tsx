    ...
    <section className="space-y-4">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-e1">
        <div className="flex items-baseline justify-between">
          ...
          <p className="text-[11px] text-slate-400">触发的调用会出现在右栏</p>
        </div>
        <div className="mt-5 space-y-2.5">
          <ScenarioBtn
            ...
            title="Trigger a real payment(via gateway)"
            subtitle="走通整条流程 · WEA 被调一次 verify 一次 settle · 真上链"
          />
          <ScenarioBtn
            running={running === "direct-verify"} disabled={!!running}
            onClick={() => onRun("direct-verify")} tone="danger"
            icon={<ShieldAlert className="h-4 w-4" />}
            title="Send bogus payload directly to /facilitator/verify"
            subtitle="绕过 gateway · 直接拿无效 signed_tx 调 WEA · 期望 isValid=false"
          />
        </div>
      </div>
...
function ResultPanel({ result }: { result: ScenarioResult }) {
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-e1 animate-flow-in">
      <div className="flex items-center justify-between">
        <h3 className="text-[12px] font-bold uppercase tracking-widest text-slate-500">最近一次场景</h3>
      ...
      {result.expected && <p className="mt-2 text-[12px] text-slate-500">期望:<span className="text-slate-700">{result.expected}</span></p>}
  ...
  return (
    <section className="space-y-3">
      <div className="flex items-baseline justify-between">
        ...
        <p className="text-[11px] text-slate-400">每 2 秒刷新 · ring buffer · 200</p>
      </div>
      {calls.length === 0 ? (
        <div className="grid-bg rounded-2xl border border-dashed border-slate-200 bg-white p-12 text-center">
          ...
          <p className="mt-3 text-[13px] text-slate-600">还没有 facilitator 调用</p>
          <p className="mt-1 text-[11px] text-slate-400">点左边 “Trigger a real payment” 触发一笔</p>
        </div>
      ) : (
    ...
    <article className={cn(
      ...
      <header className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-3 text-[11px] text-slate-500">
          ...
          <span>{new Date(c.started_at).toLocaleTimeString("zh-CN", { hour12: false })}</span>
        </div>
      </header>
