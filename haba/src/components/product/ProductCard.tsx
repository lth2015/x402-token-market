...
export function ProductCard({ product, className }: { product: MarvieProduct; className?: string }) {
  return (
    <article
      className={cn(
        "group flex flex-col rounded-2xl border border-border-subtle bg-surface-base p-6 shadow-e1 transition-shadow hover:shadow-e2",
        className,
      )}
    >
      ...
      <h3 className="mt-4 text-body font-semibold text-brand-ink">{product.shortName}</h3>
      <p className="mt-1 text-caption text-ink-tertiary">{product.sku}</p>
      <p className="mt-3 text-small text-ink-secondary">{product.shortPitch}</p>
